...
def run_lin_groupjob_locally(feat_ids, base_path, data_path, split_path, N_folds, result_path, path_number, local_path, classifier):
	...
	for feature_id in feat_ids:
		...
		#print "Shape of X_base is %s and cur_feat is %s\n" % (str(BASE_X.shape), str(cur_feat.shape))
		...
		if BASE_X.size == 0:
			...
			#print "%d is base size, " % (BASE_X.size)
			X = np.concatenate((BASE_X, cur_feat), axis=0)
		# transpose
		X = np.transpose(X)
		
		#print "X SHAPE IS %s and Y SHAPE IS %s\n" % (str(X.shape), str(y.shape))
		
		
		if classifier == 'LINREG':
			crm = linear_model.LinearRegression()
			average = np.average(cross_validation.cross_val_score(crm, X, y, cv=int(N_folds)))	# use k-fold cross validation to predict
			
		if classifier == 'LASSO':
			crm = linear_model.LassoCV(cv=3)
			average = np.average(cross_validation.cross_val_score(crm, X, y, cv=int(N_folds)))	# returns average score
...
def run_linreg_cv_locally(num, base_path, data_path, split_path, N_folds, result_path, path_number, local_path, classifier):
	...
	#corr_scorer = make_scorer(corrcoef_score_func, greater_is_better=True)
	
	if classifier == 'LINREG':
		# prepare the estimator
		lrm = linear_model.LinearRegression()
		
		# predict the cross validated values
		#fold_scores = cross_validation.cross_val_score(lrm, X, y, scoring=corr_scorer, cv=6)	# use k-fold cross validation to predict
		fold_scores = cross_validation.cross_val_score(lrm, X, y, cv=int(N_folds))	# use k-fold cross validation to predict
		
	elif classifier == 'LASSO':
		# prepare the estimator
		print "Fitting LASSO model...\n"
		lrm = linear_model.LassoCV(cv=3)
		fold_scores = cross_validation.cross_val_score(lrm, X, y, cv=int(N_folds))
...
if __name__ == "__main__":
	...
	if len(feat_id) > 1:
		run_lin_groupjob_locally(feat_id, base_path, data_path, split_path, N_folds, result_path, path_number, local_path, classifier)	# run optimized version of multiple evaluations per job
	else:
		#print "FIRST THREE NUMBERS ARE %d %d %d\n\n\n" % (int(numbers[0]), int(numbers[1]), int(numbers[2]))
		#Iterate_Feature_CVs(numbers, base_path, data_path, N_folds, result_path, path_number, local_path)
		run_linreg_cv_locally(feat_id, base_path, data_path, split_path, N_folds, result_path, path_number, local_path, classifier)
